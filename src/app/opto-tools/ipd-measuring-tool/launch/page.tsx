...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  cameraContainer: { position: 'relative', background: '#111827', borderRadius: '16px', overflow: 'hidden', aspectRatio: '4/3', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1)' },
...
const IPDMeasurement: React.FC = () => {
  ...
  const calculateFaceAngle = useCallback((landmarks: Landmark[], canvasWidth: number, canvasHeight: number): number => {
    ...
    const angle = Math.atan2((rightEye.y - leftEye.y) * canvasHeight, (rightEye.x - leftEye.x) * canvasWidth) * (180 / Math.PI);
    return angle;
  }, []);
  ...
  const isFaceCentered = useCallback((landmarks: Landmark[], canvasWidth: number, canvasHeight: number): boolean => {
    ...
    return Math.abs(noseX - centerX) < toleranceX && Math.abs(noseY - centerY) < toleranceY;
  }, []);
  ...
  const analyzeLighting = useCallback((): number => {
    ...
    tempCtx.drawImage(video, startX, startY, sampleSize, sampleSize, 0, 0, sampleSize, sampleSize);
    const imageData = tempCtx.getImageData(0, 0, sampleSize, sampleSize);
    const data = imageData.data;
    let totalBrightness = 0, leftBrightness = 0, rightBrightness = 0, topBrightness = 0, bottomBrightness = 0;
    const midX = sampleSize / 2, midY = sampleSize / 2;
    for (let i = 0; i < data.length; i += 4) {
      const brightness = (data[i] * 0.299 + data[i + 1] * 0.587 + data[i + 2] * 0.114);
      totalBrightness += brightness;
      const pixelIndex = i / 4, x = pixelIndex % sampleSize, y = Math.floor(pixelIndex / sampleSize);
      if (x < midX) leftBrightness += brightness; else rightBrightness += brightness;
      if (y < midY) topBrightness += brightness; else bottomBrightness += brightness;
    }
    const pixelCount = data.length / 4, halfPixels = pixelCount / 2;
    const avgBrightness = totalBrightness / pixelCount;
    const normalizedBrightness = Math.min(100, (avgBrightness / 180) * 100);
    const leftAvg = leftBrightness / halfPixels, rightAvg = rightBrightness / halfPixels, topAvg = topBrightness / halfPixels, bottomAvg = bottomBrightness / halfPixels;
  ...
  const calculateAccuracy = useCallback((distance: number, lighting: number, landmarks: Landmark[], faceAngle: number, stability: number, leftEyeOpen: boolean, rightEyeOpen: boolean): number => {
    let accuracy = 100;
    if (distance < MIN_DISTANCE_CM) accuracy -= (MIN_DISTANCE_CM - distance) * 3; else if (distance > MAX_DISTANCE_CM) accuracy -= (distance - MAX_DISTANCE_CM) * 2; else {
      const optimalDeviation = Math.abs(distance - IDEAL_DISTANCE_CM);
      if (optimalDeviation < 5) accuracy += 2; else accuracy -= optimalDeviation * 0.3;
    }
  ...
  const calculateMetrics = useCallback((landmarks: Landmark[]): Metrics => {
    ...
    const mmPerPixelIris = AVERAGE_IRIS_DIAMETER_MM / irisDiameterPx, mmPerPixelFace = AVERAGE_FACE_WIDTH_MM / faceWidthPx;
    ...
    const accuracy = calculateAccuracy(filteredDistance, lighting, landmarks, faceAngle, stability, isLeftEyeOpen, isRightEyeOpen);
  ...
  const calculateFinalAverage = useCallback((measurements: { ipd: number; leftPd: number; rightPd: number; accuracy: number; samples: number; confidence: number; }[]) => {
    ...
    const medianIndex = Math.floor(sortedByIpd.length / 2);
    const medianIpd = sortedByIpd[medianIndex].ipd;
    ...
    return { ipd: blendedIpd, leftPd: weightedLeftPd / totalWeight, rightPd: weightedRightPd / totalWeight, accuracy: Math.min(100, (avgAccuracy / measurements.length) + consistencyBonus), samples: totalSamples, confidence: Math.min(100, (avgConfidence / measurements.length) + consistencyBonus) };
  }, []);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        setLoadingSubtext('This may take a moment.');
        ...
        faceLandmarkerRef.current = await vision.FaceLandmarker.createFromOptions(filesetResolver, {
          baseOptions: { modelAssetPath: 'https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/1/face_landmarker.task', delegate: hasWebGPU ? 'GPU' : 'CPU' },
        ...
        if (error instanceof Error) {
            setLoadingSubtext(error.message);
        } else {
            setLoadingSubtext('Could not start camera or load model.');
        }
      }
    };
  ...
  const handleSaveResult = () => {
    if (currentMeasurement) {
      const historyItem: HistoryItem = { ipd: currentMeasurement.ipd.toFixed(1), accuracy: currentMeasurement.accuracy, timestamp: new Date().toLocaleString(), samples: currentMeasurement.samples, captures: AUTO_CAPTURE_TOTAL };
  ...
  const getMetricStatus = (value: number, type: 'ipd' | 'distance' | 'lighting'): { text: string; className: string } => {
    if (type === 'ipd') {
      if (value >= 54 && value <= 74) return { text: 'Within normal range (54-74mm)', className: 'good' };
      if (value < 54) return { text: 'Below average range', className: 'warning' };
      return { text: 'Above average range', className: 'warning' };
    }
    if (type === 'distance') {
      if (value >= MIN_DISTANCE_CM && value <= MAX_DISTANCE_CM) return { text: 'Optimal distance', className: 'good' };
      if (value < MIN_DISTANCE_CM) return { text: 'Move back slightly', className: 'warning' };
      return { text: 'Move closer', className: 'warning' };
    }
    if (type === 'lighting') {
      if (value >= 70) return { text: 'Excellent lighting', className: 'good' };
      if (value >= 50) return { text: 'Acceptable lighting', className: 'warning' };
  ...
  const getFaceGuideStyle = (): React.CSSProperties => {
    const canvas = canvasRef.current;
    if (!canvas) return { ...styles.faceGuide, top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '200px', height: '280px' };
  ...
  return (
    <div style={styles.container}>
      {isLoading ? (
        ...
        <div style={{ ...styles.cameraContainer, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'white', padding: '20px', textAlign: 'center' }}>
          <Camera size={48} style={{ marginBottom: '16px', color: '#ef4444' }} />
          <h2 style={{ fontSize: '18px', fontWeight: 600 }}>Camera Access Denied</h2>
          <p style={{ color: '#d1d5db', marginTop: '8px' }}>Please enable camera permissions in your browser settings to use this tool.</p>
        </div>
      ) : (
        <>
