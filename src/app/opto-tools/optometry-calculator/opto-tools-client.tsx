...
function IPDMeasuringTool() {
    ...
    return (
        <div className="space-y-6 ipd-tool-container">
          {loading && (
          ...
          <div className="camera-section">
            <div className="camera-container">
              ...
              <button className="capture-btn" onClick={handleCapture} disabled={!faceDetected}><ScanFace size={28}/></button>
            </div>
    
            <div className="metrics-panel">
                <MetricCard icon={<Ruler size={18} />} title="INTERPUPILLARY DISTANCE" value={`${metrics.ipd > 0 ? metrics.ipd.toFixed(1) : '--'} mm`} />
                <MetricCard icon={<Move size={18} />} title="DISTANCE FROM CAMERA" value={`${metrics.distance > 0 ? metrics.distance.toFixed(0) : '--'} cm`}>
                ...
                <MetricCard icon={<Sun size={18} />} title="LIGHTING QUALITY" value={`${metrics.lighting > 0 ? metrics.lighting : '--'}%`}>
            ...
            <div className="instructions">
                <h3><Info size={16} /> Tips for Accurate Measurement</h3>
                <ul>
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent className="modal">
                     <div className="modal-header">
                        ...
                        <h2 className="modal-title">Measurement Complete</h2>
                    </div>
...
const tools = [
  {
    ...
    description: 'Measure interpupillary distance using your device camera.',
    component: <IPDMeasuringTool />,
    category: 'refraction'
  },
  {
    id: 'vertex',
...
export function OptoToolsClient() {
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription>Select a category to filter the tools below.</CardDescription>
                 <div className="tabs-container -ml-4 -mr-4 -mb-6">
                    <div className="glass-tab-bar">
                      {categories.map(category => (
                          <button
                              key={category.id}
                              onClick={() => setActiveTab(category.id)}
                              className={cn("tab-pill flex items-center gap-2", activeTab === category.id && "active")}
                          >
                              {category.icon}
                              {category.name}
                          </button>
